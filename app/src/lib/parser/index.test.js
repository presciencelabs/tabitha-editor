...
describe('parse', () => {
	...
	describe('problems detected', () => {
		test('bad_notesNotation[badbracket you', () => {
			...
			expect(results[0].messages[0].error).toEqual(ERRORS.NO_SPACE_BEFORE_UNDERSCORE)
			expect(results[1].token).toBe('you')
			expect(results[1].messages[0].error).toMatch(/^Second person pronouns/)
			expect(results[2].token).toBe('.')
			expect(results[2].messages[0].error).toEqual(ERRORS.MISSING_PERIOD)
			expect(results[3].token).toBe(']')
			expect(results[3].messages[0].error).toEqual(ERRORS.MISSING_CLOSING_BRACKET)
		...
		test('Ok _notesNotation text[badbracket you', () => {
			...
			expect(results[2].messages[0].error).toEqual(ERRORS.NO_SPACE_BEFORE_OPENING_BRACKET)
			expect(results[3].token).toBe('badbracket')
			expect(results[3].messages.length).toBe(0)
			expect(results[4].token).toBe('you')
			expect(results[4].messages[0].error).toMatch(/^Second person pronouns/)
			expect(results[5].token).toBe('.')
			expect(results[5].messages[0].error).toEqual(ERRORS.MISSING_PERIOD)
			expect(results[6].token).toBe(']')
			expect(results[6].messages[0].error).toEqual(ERRORS.MISSING_CLOSING_BRACKET)
			expect(results).length(7)
		})
		...
		test('Ok _notesNotation [fixedbracket you(Paul', () => {
			...
			expect(results[4].messages[0].error).toEqual(ERRORS.MISSING_CLOSING_PAREN)
			expect(results[5].token).toBe('.')
			expect(results[5].messages[0].error).toEqual(ERRORS.MISSING_PERIOD)
			expect(results[6].token).toBe(']')
			expect(results[6].messages[0].error).toEqual(ERRORS.MISSING_CLOSING_BRACKET)
		...
		test('Paul explained [the Christ needed to become alive again]].', () => {
			...
			expect(results[0].token).toBe('[')
			expect(results[0].messages[0].error).toEqual(ERRORS.MISSING_OPENING_BRACKET)
			expect(results[1].token).toBe('Paul')
			expect(results[1].messages.length).toBe(0)
			expect(results[2].token).toBe('explained')
			expect(results[2].messages.length).toBe(0)
		...
		test('John said, ["what do you(person) want?" then that person took the book that John had]', () => {
			...
			expect(results[5].messages[0].error).toEqual(ERRORS.FIRST_WORD_NOT_CAPITALIZED)
			...
			expect(results[10].messages.length).toBe(0)
			expect(results[11].token).toBe(']')
			expect(results[11].messages[0].error).toEqual(ERRORS.MISSING_CLOSING_BRACKET)
			expect(results[12].token).toBe('[')
			expect(results[12].messages[0].error).toEqual(ERRORS.MISSING_OPENING_BRACKET)
			expect(results[13].token).toBe('then')
			expect(results[13].messages[0].error).toEqual(ERRORS.FIRST_WORD_NOT_CAPITALIZED)
			...
			expect(results[23].messages[0].error).toEqual(ERRORS.MISSING_PERIOD)
			expect(results).length(24)
		})
	})
})
